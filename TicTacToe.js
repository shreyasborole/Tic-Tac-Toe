...
class TicTacToe {
    ...
    #createGrid(gridWidth, gridPadding) {
        ...
        for (let i = 1; i < this.gridSize; i++) {
            const start_v = createVector(width / this.gridSize * i, 0 + gridPadding);
            const end_v = createVector(width / this.gridSize * i, height - gridPadding);
            const start_h = createVector(0 + gridPadding, height / this.gridSize * i);
            const end_h = createVector(width - gridPadding, height / this.gridSize * i);
            if (i % 2 == 0) {
                this.#gridLines.push(new AnimatedLine(end_v, start_v, color(246, 233, 233, 240), gridWidth))
                this.#gridLines.push(new AnimatedLine(end_h, start_h, color(246, 233, 233, 240), gridWidth))
            }
            else {
                this.#gridLines.push(new AnimatedLine(start_v, end_v, color(246, 233, 233, 240), gridWidth))
                this.#gridLines.push(new AnimatedLine(start_h, end_h, color(246, 233, 233, 240), gridWidth))
            }
        }
    }
        ...
        if (isAI) {
            if (this.#currentPlayer == player1) {
                if (this.#board[posx][posy] == null) {
                    this.#board[posx][posy] = new Cross(posx, posy, width / 6);
            ...
            if (this.#currentPlayer == player2) {
                this.#board[i][j] = new Circle(i, j, width / 6);
            ...
            if (this.#currentPlayer == player1) {
                ...
                if (this.#board[posx][posy] == null) {
                    this.#board[posx][posy] = new Circle(posx, posy, width / 6);
                    this.#currentPlayer = player1;
                }
            }
        }
...
#place_winning_line(a, b) {
    this.#winningLine = new AnimatedLine(a.getCenter(), b.getCenter(), color(246, 233, 233, 200), 7);
}
